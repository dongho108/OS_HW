...
void procExecSim(struct process *(*scheduler)()) {
	...
	while(1) {
		...
		if (qTime == QUANTUM ) { /* CASE 1 : The quantum expires 퀀텀시간 다됨 */
			//현재 running process는 ready queue로 감
			//스케쥴러
			//
		}
...
struct process* RRschedule() {
	//Ready Queue에 들어오는 순서대로 CPU할당, Quantum 설정, (Quanum == INT_MAX) = true 이면 FCFS와 같은결과.
